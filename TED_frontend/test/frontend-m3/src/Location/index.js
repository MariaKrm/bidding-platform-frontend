...
export default class HashtagPage extends Component {
	...
	loadPosts() {
		if (!this.query.location) return;
		request.send("GET", "/post/location?query="+this.query.location).then(res => res.data).then(posts => {
	...
	render() {
		...
		return <div className={"locations"}>
			<Jumbotron>
				<h1>Location Page &mdash; {location}</h1>
				{results.length > 0 ? <p>Here is a collection of {results.length} post{plural} for {location}</p> : <p>We are sorry. There are no posts related to {location}...</p>}
			</Jumbotron>
			{results.length > 0 &&
				<Fragment>
					<ul>
